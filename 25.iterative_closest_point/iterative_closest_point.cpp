 ...
 main (int argc, char** argv)
{
      ...
      cloud_in->points.resize (cloud_in->width * cloud_in->height); //重新设定点云的大小,

      for (size_t i = 0; i < cloud_in->points.size (); ++i) //点云填充任意数据
      {
      ...
      *cloud_out = *cloud_in;
      ...
      for (size_t i = 0; i < cloud_in->points.size (); ++i)
          cloud_out->points[i].x = cloud_in->points[i].x + 0.7f;    //x偏移0.7
